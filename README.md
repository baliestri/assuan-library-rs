...
The workspace currently provides bounded canonical S-expression parsing and encoding in `assuan-sexpr`, and protected buffers, wire parsing, framing, encoding, and pure session state machines in `assuan-protocol`, plus TCP, Unix sockets, and custom asynchronous streams in `assuan-transport`. The remaining crates and asynchronous protocol integration are under development. No package has been published.
