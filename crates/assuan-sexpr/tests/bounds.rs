...
fn atom_payloads_are_binary_not_expression_syntax() {
  ...
  assert_eq!(tree, Sexpr::List(vec![Sexpr::Atom(b"\0\xff)"), Sexpr::Atom(b"")]));
}

#[test]
fn empty_atoms_and_lists_are_valid() {
  assert_eq!(parse_complete(b"0:", ParseLimits::default()).unwrap(), Sexpr::Atom(b""));
  assert_eq!(parse_complete(b"()", ParseLimits::default()).unwrap(), Sexpr::List(vec![]));
}
...
fn prefix_reports_consumption_without_parsing_the_suffix() {
  ...
  assert_eq!(parse_complete(&input[n..], ParseLimits::default()).unwrap(), Sexpr::Atom(b"b"));
  assert_eq!(parse_prefix(b"0:garbage", ParseLimits::default()).unwrap().1, 2);
}
...
fn empty_and_truncated_input_report_eof_without_panicking() {
  for input in [b"".as_slice(), b"(", b"(0:", b"3:ab", b"1", b"(1:a", b"((1:a)"] {
    let error = parse_complete(input, ParseLimits::default()).unwrap_err();
...
fn atom_length_requires_canonical_digits_and_a_colon() {
  ...
  for input in [b":a".as_slice(), b"-1:a", b"+1:a", b")"] {
    assert_eq!(
      parse_complete(input, ParseLimits::default()).unwrap_err().kind(),
...
fn declared_length_is_limited_before_checking_for_missing_payload() {
  ...
  assert_eq!(parse_complete(b"3:abc", limits).unwrap(), Sexpr::Atom(b"abc"));
...
fn node_budget_counts_lists_and_atoms_once() {
  ...
  for input in [b"(1:a1:b0:)".as_slice(), b"((0:)())"] {
    assert_eq!(parse_complete(input, limits).unwrap_err().kind(), ParseErrorKind::NodeLimit);
  }
}
...
fn configured_maximum_depth_produces_a_tree_that_can_be_dropped() {
  let input = format!("{}0:{}", "(".repeat(MAX_NESTING_DEPTH), ")".repeat(MAX_NESTING_DEPTH));
...
fn configuration_errors_do_not_invent_input_offsets() {
  ...
  assert_eq!(parse_complete(b"()", limits).unwrap_err().kind(), ParseErrorKind::DepthLimit);
}
