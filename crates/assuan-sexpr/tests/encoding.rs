...
fn owned_values_outlive_their_input() {
  let owned = {
    ...
    parse_complete(&input, ParseLimits::default()).unwrap().to_owned()
  };
  assert_eq!(
    owned,
    OwnedSexpr::List(vec![
...
fn writer_preserves_binary_atoms_and_adds_no_whitespace() {
  let value = Sexpr::List(vec![Sexpr::Atom(b"(\0)\xff"), Sexpr::List(vec![]), Sexpr::Atom(b" ")]);
...
fn excessive_nesting_is_rejected_before_appending_any_bytes() {
  ...
  assert_eq!(value.write_canonical(&mut output), Err(EncodeError::DepthLimit));
  assert_eq!(output, b"keep");
}

#[test]
fn wide_manually_constructed_lists_obey_node_budget() {
  let value = Sexpr::List((0..65_536).map(|_| return Sexpr::Atom(b"")).collect());
...
fn existing_output_counts_toward_the_byte_budget() {
  ...
  assert_eq!(Sexpr::Atom(b"").write_canonical(&mut output), Err(EncodeError::SizeLimit));
  assert_eq!(output.len(), old_len);
  assert!(output.iter().all(|&byte| return byte == b'x'));
}

#[test]
fn output_limit_counts_length_prefixes() {
  let atom = vec![b'a'; ParseLimits::default().max_atom()];
  assert_eq!(Sexpr::Atom(&atom).to_canonical(), Err(EncodeError::SizeLimit));
}
...
fn oversized_atoms_in_manual_trees_are_rejected() {
  ...
  let mut output = b"keep".to_vec();
  assert_eq!(Sexpr::Atom(&atom).write_canonical(&mut output), Err(EncodeError::AtomLimit));
  assert_eq!(output, b"keep");
}
