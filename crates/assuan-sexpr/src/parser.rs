...
pub fn parse_prefix(input: &[u8], limits: ParseLimits) -> Result<(Sexpr<'_>, usize), ParseError> {
    ...
    loop {
        let byte = *input
            .get(position)
            .ok_or_else(|| ParseError::at(ParseErrorKind::UnexpectedEof, position))?;
        let value = match byte {
            b'(' => {
                ...
                lists
                    .try_reserve(1)
                    .map_err(|_| ParseError::at(ParseErrorKind::AllocationFailed, position))?;
                lists.push(Vec::new());
                position += 1;
                continue;
            }
            b')' => {
                let children = lists
                    .pop()
                    .ok_or_else(|| ParseError::at(ParseErrorKind::UnexpectedToken, position))?;
                position += 1;
                Sexpr::List(children)
            }
            b'0'..=b'9' => {
...
fn parse_atom<'a>(
    ...
    while let Some(&byte) = input.get(*position) {
        ...
        length = length
            ...
            .ok_or_else(|| ParseError::at(ParseErrorKind::LengthOverflow, *position))?;
        *position += 1;
    }
    ...
    let end = (*position)
        .checked_add(length)
        .ok_or_else(|| ParseError::at(ParseErrorKind::LengthOverflow, start))?;
    let payload = input
        .get(*position..end)
        .ok_or_else(|| ParseError::at(ParseErrorKind::UnexpectedEof, input.len()))?;
    *position = end;
    Ok(payload)
}
