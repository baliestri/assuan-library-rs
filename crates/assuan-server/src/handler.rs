...
///
/// One boxed future per invocation permits heterogeneous handlers in a
/// registry. Arguments may remain borrowed across awaits, but cannot outlive
/// the call.
...
/// needs `Send` because each session owns its state exclusively. Metadata
/// should remain stable after registration. Names are case-sensitive.
///
