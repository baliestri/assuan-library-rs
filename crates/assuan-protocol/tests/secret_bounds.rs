...
fn appends_and_clear_preserve_the_allocation() {
    ...
    assert_eq!(
        secret.extend_from_slice(b"overflow"),
...
fn impossible_capacity_is_a_recoverable_error() {
    assert!(matches!(
        SecretBytes::with_capacity(usize::MAX),
        Err(LimitError::AllocationFailed)
    ));
}
...
fn diagnostics_contain_only_metadata() {
    ...
    assert_eq!(
        format!("{secret:?}"),
        "SecretBytes { len: 12, capacity: 12, .. }"
    );
    assert_eq!(
        format!("{:?}", SecretRef::new(secret.expose())),
        "SecretRef { len: 12, .. }"
    );
    assert_eq!(
        format!("{:?}", PayloadRef::Public(secret.expose())),
        "Public { len: 12, .. }"
    );
    assert_eq!(
