...
impl fmt::Debug for SecretRef<'_> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    return f.debug_struct("SecretRef").field("len", &self.bytes.len()).finish_non_exhaustive();
  }
}
...
impl fmt::Debug for PayloadRef<'_> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    return f.debug_struct(variant).field("len", &len).finish_non_exhaustive();
  }
}
...
mod tests {
  ...
  fn clear_wipes_the_entire_allocation_and_preserves_it() {
    ...
    secret.clear();
    assert!(secret.storage.iter().all(|byte| return *byte == 0));
